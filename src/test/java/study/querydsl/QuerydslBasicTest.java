...
//        List<Member> content = fetchResult.getResults();
    ...
    /**
     * 1. 회원 나이 내림차순 2. 회원 이름 오름차순 3. 2에서 회원 이름이 없으면 마지막에 출력
     */
    @Test
    public void sort() {
    ...
    public void paging() {
        ...
        assertEquals(fetchResult.size(), 2);
    }
    ...
    public void paging2() {
        ...
        assertEquals(fetchResult.getTotal(), 4);
        assertEquals(fetchResult.getLimit(), 2);
        assertEquals(fetchResult.getOffset(), 1);
        assertEquals(fetchResult.getResults().size(), 2);
    }
    ...
    /**
     * 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인, 회원은 모두 조회 select m,t from Member m left join m.team t on
     * t.name = 'teamA'
     */
    @Test
    ...
    /**
     * JPA 서브쿼리는 from 절에서 안된다. 한계.
     * 이경우에는 nativeQuery를 사용해야 한다.
     */
    @Test
